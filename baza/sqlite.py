...
class Database:
    ...
    def update_language(self, telegram_id: int, new_language: str):
        sql = """
        UPDATE Users SET language = ? WHERE telegram_id = ?;
        """
        self.execute(sql, parameters=(new_language, telegram_id), commit=True)

    def get_language(self, telegram_id: int):
        sql = """
        SELECT language FROM users WHERE telegram_id = ?;
        """

        return  self.execute(sql, parameters=(telegram_id,), fetchone=True)[0]
